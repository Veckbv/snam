...
def confirm(token):
    ...
    if current_user.confirm(token):
        ...
        flash('Ссылка подтверждения недействительна или истекла.')
    return redirect(url_for('main.index'))
...
def resend_confirmation():
    ...
    flash('Новое электронное письмо с подтверждением отправлено вам по электронной почте.')
    return redirect(url_for('main.index'))
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            send_email(user.email, 'Сбросить пароль',
                       ...
                       next=request.args.get('next'))
        flash('Вам отправлено электронное письмо с инструкциями по сбросу пароля.')
        return redirect(url_for('auth.login'))
...
def change_email(token):
    if current_user.change_email(token):
        db.session.commit()
        flash('Ваш адрес электронной почты обновлен.')
    else:
